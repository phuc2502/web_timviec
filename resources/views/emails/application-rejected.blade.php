...
<div class="wrapper">
  ...
  {{-- Body --}}
  <div class="body">
    <p class="greeting">Kính gửi {{ $application->user->name }},</p>

    <p class="text">
      Đầu tiên, <strong>{{ $application->listing->user->company_name ?? $application->listing->user->name }}</strong>
    ...
    <div class="highlight-box">
      ...
      <div class="row">
        ...
        <span class="value">{{ $application->applied_at?->format('d/m/Y') ?? 'N/A' }}</span>
      </div>
    </div>
    ...
    <div class="encouragement-box">
      ...
      <ul>
        <li>Tiếp tục khám phá các cơ hội việc làm phù hợp khác trên <strong>{{ config('app.name') }}</strong></li>
    ...
    <p class="text">
      ...
      Một lần nữa, <strong>cảm ơn bạn đã tin tưởng và lựa chọn {{ $application->listing->user->company_name ?? $application->listing->user->name }}</strong>.
    </p>
    ...
    <div class="cta">
      <a href="{{ url('/job') }}">Tìm kiếm cơ hội khác</a>
    </div>
