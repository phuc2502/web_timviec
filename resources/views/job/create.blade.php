          ...
          <div class="flex-col gap-16">
            ...
            <div class="form-group">
              ...
              <input type="text" name="title" class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}"
                placeholder="VD: Senior Backend Developer (Node.js)" value="{{ old('title') }}" required>
            ...
            <div class="form-group">
              ...
              <textarea name="description" id="description" class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}"
                rows="8" placeholder="Mô tả chi tiết về công việc, dự án, môi trường làm việc..." required>{{ old('description') }}</textarea>
              @error('description')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>

            <div class="form-group">
              <label class="form-label">Yêu cầu ứng viên <span class="required">*</span></label>
              <textarea name="roles" class="form-control {{ $errors->has('roles') ? 'is-invalid' : '' }}"
                rows="6" placeholder="- Kinh nghiệm 2+ năm với Node.js&#10;- Thành thạo RESTful API&#10;- Biết Docker, CI/CD là lợi thế" required>{{ old('roles') }}</textarea>
            ...
            <div class="form-group">
              ...
              <textarea name="predes" class="form-control" rows="4"
                placeholder="- Lương thưởng cạnh tranh&#10;- 13 tháng lương&#10;- Review lương 2 lần/năm">{{ old('predes') }}</textarea>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="card-body" style="padding:20px">
          <div class="flex-col gap-14">
            ...
            <div class="form-group">
              ...
              <input type="number" name="salary" class="form-control {{ $errors->has('salary') ? 'is-invalid' : '' }}"
                placeholder="0 = Thỏa thuận" value="{{ old('salary', 0) }}" min="0">
            ...
            <div class="form-group">
              ...
              <select name="address" class="form-control {{ $errors->has('address') ? 'is-invalid' : '' }}" required>
                <option value="">-- Chọn địa điểm --</option>
                @foreach(['Hà Nội','Hồ Chí Minh','Đà Nẵng','Cần Thơ','Remote','Toàn quốc'] as $loc)
                  <option value="{{ $loc }}" {{ old('address') == $loc ? 'selected' : '' }}>{{ $loc }}</option>
                @endforeach
              </select>
            ...
            <div class="form-group">
              ...
              <select name="job_type" class="form-control {{ $errors->has('job_type') ? 'is-invalid' : '' }}" required>
