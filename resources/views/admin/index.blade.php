...
<h2 style="font-size:13px; font-weight:800; text-transform:uppercase; letter-spacing:.08em; color:#94a3b8; margin:0 0 14px 0;">
  <i class="fas fa-chart-line" style="margin-right:6px;"></i> Biểu đồ thống kê
</h2>

{{-- Row 1: Line chart (Users + Jobs) & Doughnut (Plan) --}}
<div style="display:grid; grid-template-columns:2fr 1fr; gap:16px; margin-bottom:16px;">

  {{-- Line chart: Users & Jobs theo tháng --}}
  <div class="card" style="padding:20px;">
    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:16px;">
      <span style="font-size:13px; font-weight:700; color:#1e293b;">
        <i class="fas fa-users" style="color:#3b82f6; margin-right:6px;"></i>
        Người dùng &amp; Tin tuyển dụng mới theo tháng
      </span>
    </div>
    <canvas id="lineChart" style="max-height:240px;"></canvas>
  </div>

  {{-- Doughnut: Phân loại gói dịch vụ --}}
  <div class="card" style="padding:20px;">
    <div style="margin-bottom:16px;">
      <span style="font-size:13px; font-weight:700; color:#1e293b;">
        <i class="fas fa-crown" style="color:#8b5cf6; margin-right:6px;"></i>
        Phân loại gói dịch vụ
      </span>
    </div>
    <canvas id="planChart" style="max-height:200px;"></canvas>
...
{{-- Row 2: Bar chart doanh thu --}}
<div class="card" style="padding:20px; margin-bottom:20px;">
  <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:16px;">
    <span style="font-size:13px; font-weight:700; color:#1e293b;">
      <i class="fas fa-coins" style="color:#f59e0b; margin-right:6px;"></i>
      Doanh thu theo tháng (VNĐ)
    </span>
    <span style="font-size:11px; color:#94a3b8;">12 tháng gần nhất</span>
  </div>
  <canvas id="revenueChart" style="max-height:220px;"></canvas>
</div>

{{-- Row 3: Trạng thái tin tuyển dụng --}}
<div style="display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-bottom:20px;">

  {{-- Bar: Phân loại users (employee vs employer) --}}
  <div class="card" style="padding:20px;">
    <div style="margin-bottom:16px;">
      <span style="font-size:13px; font-weight:700; color:#1e293b;">
        <i class="fas fa-user-friends" style="color:#10b981; margin-right:6px;"></i>
        Phân loại thành viên
      </span>
    </div>
    <canvas id="userTypeChart" style="max-height:180px;"></canvas>
  </div>

  {{-- Bar: Trạng thái tin tuyển dụng --}}
  <div class="card" style="padding:20px;">
    <div style="margin-bottom:16px;">
      <span style="font-size:13px; font-weight:700; color:#1e293b;">
        <i class="fas fa-briefcase" style="color:#f97316; margin-right:6px;"></i>
        Trạng thái tin tuyển dụng
      </span>
    </div>
    <canvas id="jobStatusChart" style="max-height:180px;"></canvas>
  </div>
...
(function() {
  ...
  new Chart(document.getElementById('lineChart'), {
    ...
    options: {
      responsive: true,
      plugins: {
        legend: { labels: { font, boxWidth: 12 } },
  ...
  new Chart(document.getElementById('planChart'), {
    ...
    data: {
      ...
      datasets: [{
        data: [{{ $freeUsers }}, {{ $trialUsers }}, {{ $premiumUsers }}],
        backgroundColor: ['#94a3b8', '#06b6d4', '#8b5cf6'],
        borderWidth: 2,
        borderColor: '#fff',
        hoverOffset: 8,
      }],
  ...
  // ── 4. Bar ngang: Phân loại user ────────────────────────────────────────
  new Chart(document.getElementById('userTypeChart'), {
    type: 'bar',
    data: {
      labels: ['Ứng viên', 'Nhà tuyển dụng', 'Bị khóa', 'Hoạt động'],
      datasets: [{
        data: [{{ $totalEmployees }}, {{ $totalEmployers }}, {{ $bannedUsers }}, {{ $activeUsers }}],
        backgroundColor: ['#10b981','#3b82f6','#ef4444','#06b6d4'],
        borderRadius: 5,
        borderWidth: 0,
      }],
    },
    options: {
      ...
      scales: {
        x: { grid: { color: gridColor }, ticks: { font, precision: 0 }, beginAtZero: true },
        y: { grid: { color: 'transparent' }, ticks: { font } },
      },
    },
  });

  // ── 5. Bar: Trạng thái tin tuyển dụng ──────────────────────────────────
  new Chart(document.getElementById('jobStatusChart'), {
    type: 'bar',
    data: {
      labels: ['Đang mở', 'Bị ẩn', 'Đã đóng'],
      datasets: [{
        data: [{{ $openJobs }}, {{ $hiddenJobs }}, {{ $closedJobs }}],
    ...
    options: {
      ...
      plugins: { legend: { display: false } },
      scales: {
        x: { grid: { color: 'transparent' }, ticks: { font } },
        y: { grid: { color: gridColor }, ticks: { font, precision: 0 }, beginAtZero: true },
      },
    },
  });
