  ...
  <aside class="admin-sidebar" id="adminSidebar">
    ...
    <nav class="sidebar-nav">
      ...
      <a href="{{ url('/admin/jobs') }}"
         ...
         class="sidebar-link {{ request()->is('admin/jobs') ? 'active' : '' }}">
        <span class="sidebar-link__icon"><i class="fas fa-briefcase fa-fw"></i></span>
        <span class="sidebar-link__label">Tin tuyển dụng</span>
      </a>

      @php $_pendingCount = \App\Models\Listing::where('status','pending')->orWhereNull('status')->count(); @endphp
      <a href="{{ url('/admin/jobs/pending') }}"
         data-tooltip="Duyệt tin{{ $_pendingCount > 0 ? ' ('.$_pendingCount.')' : '' }}"
         class="sidebar-link {{ request()->is('admin/jobs/pending*') ? 'active' : '' }}"
         style="position:relative;">
        <span class="sidebar-link__icon" style="position:relative;">
          <i class="fas fa-clipboard-check fa-fw"></i>
          @if($_pendingCount > 0)
            <span style="position:absolute;top:-6px;right:-8px;min-width:16px;height:16px;padding:0 4px;background:#ef4444;color:#fff;border-radius:8px;font-size:9px;font-weight:700;display:flex;align-items:center;justify-content:center;line-height:1;">{{ $_pendingCount }}</span>
          @endif
        </span>
        <span class="sidebar-link__label">Duyệt tin
          @if($_pendingCount > 0)
            <span style="display:inline-flex;align-items:center;justify-content:center;min-width:18px;height:18px;padding:0 5px;background:#ef4444;color:#fff;border-radius:9px;font-size:10px;font-weight:700;margin-left:6px;">{{ $_pendingCount }}</span>
          @endif
        </span>
      </a>
