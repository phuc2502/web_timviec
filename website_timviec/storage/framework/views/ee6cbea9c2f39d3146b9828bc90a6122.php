<?php $__env->startSection('title', 'Dashboard — Nhà tuyển dụng'); ?>
...
<div class="hero-employer">
  <div>
    ...
    <p>Tổng quan hoạt động tuyển dụng của <?php echo e(auth()->user()->company_name ?? 'doanh nghiệp bạn'); ?></p>
  </div>
  <a href="<?php echo e(url('/job/create')); ?>" class="btn-white"><i class="fas fa-plus" style="margin-right:6px"></i>Đăng tin mới</a>
</div>
...
<div class="trial-alert">
  ...
  <span style="font-size:14px;color:#92400e">
    Bạn đang dùng thử miễn phí — còn <strong><?php echo e(now()->diffInDays(auth()->user()->user_trial)); ?> ngày</strong>.
    <a href="<?php echo e(url('/subscribe')); ?>" style="color:#d97706;font-weight:700;text-decoration:underline">Nâng cấp Premium ngay →</a>
  </span>
</div>
<?php endif; ?>
...
<div class="stat-grid-4">
  <div class="stat-box">
    ...
    <div><div class="stat-box__num"><?php echo e($totalJobs ?? 0); ?></div><div class="stat-box__lbl">Tin đăng</div></div>
  </div>
  <div class="stat-box">
    <div class="stat-box__icon" style="background:#eff6ff;color:#3b82f6"><i class="fas fa-users"></i></div>
    <div><div class="stat-box__num"><?php echo e($totalApplicants ?? 0); ?></div><div class="stat-box__lbl">Ứng viên nộp</div></div>
  </div>
  <div class="stat-box">
    <div class="stat-box__icon" style="background:#fff7ed;color:#f59e0b"><i class="fas fa-star"></i></div>
    <div><div class="stat-box__num"><?php echo e($shortlisted ?? 0); ?></div><div class="stat-box__lbl">Shortlisted</div></div>
  </div>
  <div class="stat-box">
    <div class="stat-box__icon" style="background:#fef2f2;color:#ef4444"><i class="fas fa-fire"></i></div>
    <div><div class="stat-box__num"><?php echo e($activeJobs ?? 0); ?></div><div class="stat-box__lbl">Đang tuyển</div></div>
  </div>
</div>
...
<div class="main-grid">
  ...
  <div class="section-card">
    <div class="section-card__header">
      ...
      <a href="<?php echo e(url('/job/manage')); ?>" style="font-size:13px;color:#3b82f6;font-weight:600;text-decoration:none">Quản lý tất cả →</a>
    </div>
    <?php if(($recentJobs ?? collect())->isNotEmpty()): ?>
        ...
        <tr>
          <td>
            <a href="<?php echo e(url('/job/show/'.$job->slug)); ?>" target="_blank"
              ...
              <?php echo e(Str::limit($job->title, 38)); ?>

            </a>
          </td>
          <td style="text-align:center">
            <span class="count-badge"><?php echo e($job->users->count()); ?></span>
          </td>
    ...
    <div class="empty-state">
      ...
      <p>Bạn chưa đăng tin tuyển dụng nào.<br>
        <a href="<?php echo e(url('/job/create')); ?>" style="color:#3b82f6;font-weight:600">Đăng tin đầu tiên →</a>
      </p>
    </div>
    <?php endif; ?>
  </div>
  ...
  <div>
    ...
    <div class="plan-card">
      <div class="plan-card__body">
        ...
        <?php if(auth()->user()->billing_ends && auth()->user()->billing_ends > now()): ?>
          <div class="plan-card__name" style="color:#7effc4"><?php echo e(ucfirst(auth()->user()->plan)); ?> Premium</div>
          <div class="plan-card__meta">Hạn dùng: <?php echo e(auth()->user()->billing_ends->format('d/m/Y')); ?></div>
        <?php elseif(auth()->user()->user_trial && auth()->user()->user_trial > now()): ?>
          <div class="plan-card__name" style="color:#fcd34d">Dùng thử (Trial)</div>
          <div class="plan-card__meta">Còn lại: <?php echo e(now()->diffInDays(auth()->user()->user_trial)); ?> ngày</div>
        <?php else: ?>
      ...
      <?php if(!auth()->user()->billing_ends || auth()->user()->billing_ends <= now()): ?>
      <a href="<?php echo e(url('/subscribe')); ?>" class="plan-card__btn"><i class="fas fa-rocket" style="margin-right:6px"></i>Nâng cấp Premium</a>
      <?php endif; ?>
    </div>
    ...
    <div class="section-card">
      ...
      <div style="padding:14px">
        <a href="<?php echo e(url('/job/create')); ?>"  class="quick-btn quick-btn-primary"><i class="fas fa-plus"></i> Đăng tin tuyển dụng</a>
        <a href="<?php echo e(url('/applicants')); ?>"  class="quick-btn"><i class="fas fa-users"></i> Danh sách ứng viên</a>
        <a href="<?php echo e(url('/job/manage')); ?>"  class="quick-btn"><i class="fas fa-cog"></i> Quản lý tin đăng</a>
        <a href="<?php echo e(url('/messages')); ?>"    class="quick-btn"><i class="fas fa-comment-dots"></i> Tin nhắn</a>
        <a href="<?php echo e(url('/user/profile')); ?>" class="quick-btn"><i class="fas fa-building"></i> Hồ sơ công ty</a>
      </div>
    </div>
  </div>
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH F:\dl\website_timviec_v13_social_verified\website_output\resources\views/dashboard/employer.blade.php ENDPATH**/ ?>