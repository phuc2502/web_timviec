...
<div class="container section" style="max-width:760px">
  ...
  <div class="flex-between mb-24">
    <div>
      ...
      <p class="text-muted fs-13 mt-8">{{ $application->listing->title }}</p>
    </div>
    <a href="{{ route('candidate.history') }}" class="btn btn-outline btn-sm">
  ...
  <div style="border:2px solid {{ $s['border'] }};border-radius:var(--radius-lg);background:{{ $s['bg'] }};padding:16px 20px;margin-bottom:20px">
    <div class="flex-between">
      <div class="flex gap-12" style="align-items:center">
        <i class="fas {{ $s['icon'] }} fa-lg" style="color:{{ $s['color'] }}"></i>
        <div>
          <div class="fw-700" style="color:{{ $s['color'] }};font-size:15px">{{ $s['label'] }}</div>
          @if($application->status_updated_at)
            <div class="fs-12" style="color:var(--text-secondary);margin-top:2px">
              Cập nhật {{ $application->status_updated_at->format('d/m/Y H:i') }}
            </div>
          @endif
        </div>
      </div>
  ...
  <div style="display:flex;flex-direction:column;gap:16px">
    ...
    <div class="card">
      ...
      <div class="card-body">
        <div class="flex gap-14" style="align-items:center">
          ...
          <div>
            <div class="fw-700" style="color:var(--secondary);font-size:16px">{{ $application->listing->title }}</div>
            <div class="fs-13 text-muted mt-8">
              {{ $application->listing->user->company_name ?? $application->listing->user->name }}
            ...
            @if($application->listing->salary)
              <div class="fs-13 mt-8" style="color:var(--primary);font-weight:600">
                ...
                {{ number_format($application->listing->salary) }}đ/tháng
              </div>
            @endif
          </div>
        </div>
        <div style="margin-top:14px;padding-top:14px;border-top:1px solid var(--border)">
          <a href="{{ url('/job/show/'.$application->listing->slug) }}" class="btn btn-outline btn-sm" target="_blank">
      ...
      <div class="card">
        ...
        <div class="card-body">
          <div class="flex gap-12" style="align-items:center">
            ...
            <div style="flex:1">
              <div class="fw-600">{{ $application->cv->original_name }}</div>
              <div class="text-muted fs-12 mt-8">File CV đã nộp</div>
            </div>
            <a href="{{ Storage::url($application->cv->file_path) }}" target="_blank" class="btn btn-outline btn-sm">
    ...
    @if($application->cover_letter)
      <div class="card">
        ...
        <div class="card-body">
          <p style="font-size:14px;line-height:1.8;color:var(--text-body);white-space:pre-line">{{ $application->cover_letter }}</p>
        </div>
      </div>
    @else
      <div class="card">
  ...
  {{-- Ứng tuyển lại nếu chưa accepted/rejected --}}
  @if(!in_array($application->status, ['accepted', 'rejected']))
    <div class="text-center mt-20">
      <a href="{{ route('apply.form', ['listingId' => $application->listing_id]) }}" class="btn btn-outline">
