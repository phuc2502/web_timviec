...
<div class="flex-between mb-20">
  <div>
    ...
    <p class="text-muted fs-13 mt-2">Tổng số: <strong><?php echo e($users->total()); ?></strong> thành viên hệ thống</p>
  </div>
  <form action="<?php echo e(url('/admin/users')); ?>" method="GET">
    <div style="display:flex; gap:8px; align-items:center;">
      <input type="text" name="search" class="form-control" style="width:220px; font-size:13px;"
        placeholder="Tìm theo tên, email..." value="<?php echo e(request('search')); ?>">
      <select name="type" class="form-control" style="width:140px; font-size:13px; cursor:pointer;">
        <option value="">Tất cả vai trò</option>
        <option value="employee" <?php echo e(request('type') == 'employee' ? 'selected' : ''); ?>>Ứng viên</option>
        <option value="employer" <?php echo e(request('type') == 'employer' ? 'selected' : ''); ?>>Nhà tuyển dụng</option>
      ...
      <?php if(request()->anyFilled(['search','type'])): ?>
        <a href="<?php echo e(url('/admin/users')); ?>" class="btn btn-light btn-sm" style="height:38px; padding:0 12px;">
  ...
  <table class="table" style="vertical-align:middle;">
    ...
    <tbody>
      <?php $__empty_1 = true; $__currentLoopData = $users; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $user): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        <tr>
          <td class="text-muted fs-12 fw-700">#<?php echo e($user->id); ?></td>
          <td>
          ...
          <td>
            <form action="<?php echo e(url('/admin/users/'.$user->id.'/role')); ?>" method="POST" style="margin:0;">
              <?php echo csrf_field(); ?>
              <select name="user_type" class="form-control"
                style="font-size:11px; padding:4px 8px; height:auto; cursor:pointer;" onchange="this.form.submit()">
                <option value="employee" <?php echo e($user->user_type === 'employee' ? 'selected' : ''); ?>>👤 Ứng viên</option>
                <option value="employer" <?php echo e($user->user_type === 'employer' ? 'selected' : ''); ?>>🏢 Doanh nghiệp</option>
          ...
          <td>
            <form action="<?php echo e(url('/admin/users/'.$user->id.'/plan')); ?>" method="POST" style="margin:0;">
              <?php echo csrf_field(); ?>
              <select name="plan" class="form-control"
                style="font-size:11px; padding:4px 8px; height:auto; cursor:pointer;" onchange="this.form.submit()">
                <option value="free"    <?php echo e((empty($user->plan) || $user->plan === 'free') ? 'selected' : ''); ?>>Mặc định (Free)</option>
                <option value="trial"   <?php echo e($user->plan === 'trial'   ? 'selected' : ''); ?>>Dùng thử (Trial)</option>
                <option value="premium" <?php echo e($user->plan === 'premium' ? 'selected' : ''); ?>>👑 Premium</option>
              </select>
            </form>
          </td>
          <td class="text-muted fs-12"><?php echo e($user->created_at->format('d/m/Y')); ?></td>
          <td style="text-align:center;">
            <div style="display:flex; gap:6px; justify-content:center;">
              
              <button onclick="openUserModal(<?php echo e($user->id); ?>)" title="Xem chi tiết"
              ...
              <form action="<?php echo e(url('/admin/users/'.$user->id.'/ban')); ?>" method="POST" style="margin:0;">
                <?php echo csrf_field(); ?>
                <button type="submit"
                  onclick="return confirm('<?php echo e($user->is_banned ? 'MỞ KHÓA' : 'KHÓA'); ?> tài khoản [<?php echo e($user->name); ?>]?')"
                  title="<?php echo e($user->is_banned ? 'Mở khóa' : 'Khóa tài khoản'); ?>"
                  style="border:none; background:<?php echo e($user->is_banned ? '#ecfdf5' : '#fef2f2'); ?>; color:<?php echo e($user->is_banned ? '#10b981' : '#ef4444'); ?>; border-radius:6px; padding:5px 10px; font-size:12px; cursor:pointer; font-weight:600; transition:all .15s;"
                  onmouseover="this.style.opacity='.75';" onmouseout="this.style.opacity='1';">
                  <i class="fas <?php echo e($user->is_banned ? 'fa-lock-open' : 'fa-ban'); ?>"></i>
                </button>
              </form>
            </div>
          </td>
  ...
  <?php if($users->hasPages()): ?>
    <div class="card-footer" style="background:#f8fafc; border-top:1px solid var(--border);">
      <div class="flex-between">
        <span class="text-muted fs-13">Đang xem <?php echo e($users->firstItem()); ?>–<?php echo e($users->lastItem()); ?> / <?php echo e($users->total()); ?></span>
        <div class="pagination">
          <?php if(!$users->onFirstPage()): ?><a href="<?php echo e($users->previousPageUrl()); ?>"><i class="fas fa-chevron-left"></i></a><?php endif; ?>
          ...
          <?php if($users->hasMorePages()): ?><a href="<?php echo e($users->nextPageUrl()); ?>"><i class="fas fa-chevron-right"></i></a><?php endif; ?>
        </div>
      </div>
    </div>
  <?php endif; ?>
...
<script>
const USER_DETAIL_URL = '<?php echo e(url("/admin/users")); ?>';
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH F:\dl\web_timviec-main (3)\web_timviec-main\resources\views/admin/users.blade.php ENDPATH**/ ?>